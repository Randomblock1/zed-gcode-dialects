...
module.exports = grammar({
  ...
  conflicts: ($) => [[$.parenthesized_expression, $.parenthesized_comment]],
  ...
  rules: {
    ...
    percent_line: ($) => prec.right(seq("%", optional($.semicolon_comment), $._newline)),
    ...
    klipper_option: ($) =>
      prec(
        ...
        prec.right(seq(
          ...
          repeat(choice($._template_item, $.bare_argument, $.string, $.number, $.comma, $.colon)),
          optional($.hash_comment),
          $._newline,
        )),
      ),
    ...
    o_statement: ($) =>
      prec.right(seq(
        ...
        repeat(choice($.bracket_expression, $.parameter_reference, $.number, $.bare_argument)),
        optional($.semicolon_comment),
        $._newline,
      )),
    ...
    _command: ($) => choice($.g_code, $.m_code, $.tool_code, $.extended_command),
    ...
    axis_word: (_) => token(prec(7, /[XYZABCUVWExyzabcuvwe][-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?/)),
    feed_word: (_) => token(prec(7, /[Ff][-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?/)),
    spindle_word: (_) => token(prec(7, /[Ss][-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?/)),
    parameter_word: (_) => token(prec(6, /[A-Za-z][-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?/)),
    expression_word: ($) =>
      choice(
        seq(field("prefix", $.brace_word_start), optional(commaSep1($._expression)), "}"),
        seq(field("prefix", $.bracket_word_start), optional(commaSep1($._expression)), "]"),
    ...
    parameter_assignment: ($) =>
      seq(field("target", $.parameter_reference), "=", field("value", $._expression)),
    ...
    jinja_directive: ($) =>
      choice(
        ...
        "endmacro",
        seq(choice("filter", "with", "block", "extends", "include", "import", "from", "do"), repeat($._template_item)),
        choice("endfilter", "endwith", "endblock", "raw", "endraw", "break", "continue"),
      ),
    jinja_comment_inline: ($) => seq("{#", optional($.comment_text), "#}"),
    _template_item: ($) => choice($.brace_expression, $.jinja_statement_inline, $.jinja_comment_inline),

    _assignable: ($) => choice($.identifier, $.parameter_reference, $.reference_expression),
    ...
    slice_expression: ($) =>
      seq(optional($._expression), ":", optional($._expression), optional(seq(":", optional($._expression)))),
    call_expression: ($) =>
      prec.left(
        PREC.POSTFIX,
        seq(
    ...
    argument_list: ($) => seq("(", optional(commaSep1(choice($._expression, $.keyword_argument))), optional(","), ")"),
    keyword_argument: ($) => seq($.identifier, "=", $._expression),

    unary_expression: ($) =>
      prec(PREC.UNARY, seq(field("operator", choice("!", "-", "+", "not", "NOT", "#")), field("operand", $._expression))),
    ...
    conditional_expression: ($) =>
      choice(
        ...
        prec.right(PREC.CONDITIONAL, seq($._expression, "if", $._expression, "else", $._expression)),
      ),
    ...
    number: (_) => token(/[-+]?(?:(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?|0[xX][0-9A-Fa-f]+)/),
    string: ($) => choice($.double_quoted_string, $.single_quoted_string),
    double_quoted_string: (_) => token(seq('"', repeat(choice(/[^"\\\r\n]/, /\\./)), '"')),
