...
Validation was run on 2026-08-21 with Tree-sitter CLI 0.25.10. Every file was parsed with the generated `practical_gcode` parser; a success means the syntax tree contained no `ERROR` or missing nodes.
...
The repository-owned gates add:

- five syntax-tree corpus tests covering RRF, Klipper/Jinja, Marlin/RepRap, and LinuxCNC constructs;
- six complete example fixtures, including `examples/rrf.g`, which is based on the reported `measure_idle_window.g` failure case;
- compilation of all Zed Tree-sitter query files;
- native and WebAssembly parser builds.
